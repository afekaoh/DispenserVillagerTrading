...
public class DispernserVillagerListener implements Listener {
	...
	private final boolean enableVillagerInteraction;
	
	public DispernserVillagerListener(boolean enableVillagerInteraction) {
		this.enableVillagerInteraction = enableVillagerInteraction;
	}
	...
	private static @NotNull List<MerchantRecipe> getMatchingTrades(List<MerchantRecipe> trades, List<ItemStack> dispenserContent) {
		// since we want to include tades that require two ingredients we have to filter the trades based on the dispenser contents
		return trades
			.stream()
	...
	private static void processTrade(Dispenser dispenser,
		...
		processVillagerTradeEffects(selectedTrade, villager);
		
	}
	
	private static void processVillagerTradeEffects(MerchantRecipe selectedTrade, Villager villager) {
	...
	private static void processTradeResult(Dispenser dispenser, MerchantRecipe selectedTrade, BlockFace direction) {
	...
	private static void processDispenserPayment(MerchantRecipe selectedTrade, Inventory dispenserInventory) {
	...
	private static @NotNull Map<Material, Integer> convertDispenserContentToMap(List<ItemStack> dispenserItems) {
	...
	private static @NotNull Map<Material, Integer> convertTradesToMap(MerchantRecipe trade) {
	...
	private static @Nullable Villager getVillagerFromBlock(Location nextBlock) {
